...
source ./build.sh $_DEVICE $BUILD_TYPE
...
if [[ $BUILD_DELTA -eq 1 ]]; then
    ...
    ota_from_target_files -k ~/.android-certs/releasekey --block --backup=true  -i ${build_device}-signed-target_files_old.zip ${build_device}-signed-target_files.zip ${build_device}-signed-delta.zip
else
    echo "Building full package"
    ota_from_target_files -k ~/.android-certs/releasekey --block --backup=true ${build_device}-signed-target_files.zip lineage-${build_device}_signed.zip
fi
